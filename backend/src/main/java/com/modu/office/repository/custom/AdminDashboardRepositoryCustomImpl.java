...
public class AdminDashboardRepositoryCustomImpl implements AdminDashboardRepositoryCustom {
        ...
        return byFloor.entrySet().stream()
                .map(entry -> {
                    ...
                    int occupied = (int) floorRooms.stream()
                            ...
                            .count();
                    double rate = total == 0 ? 0.0 : Math.round((double) occupied / total * 1000) / 10.0;
                    return new OccupancyResponse(officeId, floorKey == 0 ? null : floorKey, total, occupied, rate);
                })
    ...
    public CancellationStatsResponse getCancellationStats(Long officeId, LocalDate startDate, LocalDate endDate) {
        long total = queryFactory
                ...
                .fetchOne() != null
                        ? queryFactory.select(reservation.count()).from(reservation)
                                .where(officeIdEq(officeId), dateBetween(startDate, endDate)).fetchOne()
                        : 0L;
        ...
        long canceled = queryFactory
                ...
                .fetchOne() != null
                        ? queryFactory.select(reservation.count()).from(reservation)
                                .where(officeIdEq(officeId), reservation.status.eq(ReservationStatus.CANCELED),
                                        dateBetween(startDate, endDate))
                                .fetchOne()
                        : 0L;
