    ...
    class GetAllLogs {
        ...
        void getAllLogs_Success() throws Exception {
            given(updateLogService.getAllLogs(any(Pageable.class)))
                    .willReturn(new PageImpl<>(List.of(createUpdateLogResponse()), PageRequest.of(0, 20), 1));
            ...
            mockMvc.perform(get("/api/logs")
                    ...
                    .andDo(document("update-log-all",
                            ...
                            responseFields(
                                    ...
                                    fieldWithPath("data.content[].reservationTitle").type(JsonFieldType.STRING)
                                            .description("예약 제목"),
                                    fieldWithPath("data.content[].action").type(JsonFieldType.STRING)
                                            .description("로그 액션형 (CREATE, UPDATE, CANCEL)"),
                                    fieldWithPath("data.content[].actorId").type(JsonFieldType.NUMBER)
                                            .description("수행자 회원 ID"),
                                    fieldWithPath("data.content[].actorName").type(JsonFieldType.STRING)
                                            .description("수행자 이름"),
                                    subsectionWithPath("data.content[].beforeData").type(JsonFieldType.OBJECT)
                                            .description("변경 전 JSONB 데이터 (optional)").optional(),
                                    subsectionWithPath("data.content[].afterData").type(JsonFieldType.OBJECT)
                                            .description("변경 후 JSONB 데이터"),
                                    fieldWithPath("data.content[].occurredAt").type(JsonFieldType.STRING)
                                            .description("발생 일시"),
                                    subsectionWithPath("data.pageable").type(JsonFieldType.OBJECT)
                                            .description("페이징 정보"),
                                    ...
                                    subsectionWithPath("data.sort").type(JsonFieldType.OBJECT).ignored(),
        ...
        void getLogsByReservation_Success() throws Exception {
            ...
            mockMvc.perform(get("/api/logs/reservation/{reservationId}", 100L)
                    ...
                    .andDo(document("update-log-by-reservation",
                            pathParameters(
                                    parameterWithName("reservationId").description("대상 예약 ID")),
                            queryParameters(
